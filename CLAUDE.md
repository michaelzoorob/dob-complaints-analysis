...
- **Blog prose:** every quoted number must be WRITTEN BY a committed script TO a CSV in `data/analysis/risk_models/`, windowed to match the post (2020 through May 2026 unless stated) — ad-hoc session computations don't count; headline modeled estimates quote 95% confidence intervals inline ("9% more (95% confidence interval 5 to 13%)"); never reference earlier or partial versions of an analysis; adversarial replication checks live in `scripts/audit/`.
- **Banned vocabulary in reader-facing prose:** "ledger" for violation datasets (say "violation records" or "receive fewer violations"), latent-compliance claims ("actually violate the code"), recap-tease closers ("The difference shows up in..."), future-post promises.
