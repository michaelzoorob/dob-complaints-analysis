...
def build_tract_frame(ev: pd.DataFrame, tract: pd.DataFrame) -> pd.DataFrame:
    ...
    cnt = pd.DataFrame({
        "n_disc_constr": ag[disc & (ag["category_prefix"] != "7G")]
        .groupby("tract").size(),
    ...
    for c in ("n_disc_constr", "n_7g", "n_stat", "job_months", "units_res"):
        df[c] = df[c].fillna(0)
...
ALLOC_MODELS = [
    # outcome, label, offset column, offset label
    ("n_disc_constr", "discretionary construction (8A/1X/91..., excl 7G)",
     "log_job_months", "log active job-months 2020-26"),
...
def run_allocation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for outcome, label, off, off_label in ALLOC_MODELS:
        sub = df[df[off].notna()]
        t0 = time.time()
        ...
        print(f"  {outcome:<14} offset {off_label:<28} n={m._N:,} "
...
def pick(est, part, outcome, offset_label=None):
...
def make_figure(est: pd.DataFrame, meta: dict) -> None:
    ...
    # Panel A: allocation, each margin against its own exposure, each
    # paired with the statutory contrast under the identical offset
    ax = axes[0]
    ax.axvline(0, color=ZERO_C, linewidth=1.1, zorder=1)
    draw(ax, pick(est, "allocation", "n_disc_constr"), BLUE, +0.27,
         "discretionary construction (per job-month)")
    draw(ax, pick(est, "allocation", "n_stat",
                  "log active job-months 2020-26"), BLUE, +0.09,
         "statutory, same job-month offset", contrast=True)
    draw(ax, pick(est, "allocation", "n_7g"), AQUA, -0.09,
         "7G sweeps (per residential unit)")
    draw(ax, pick(est, "allocation", "n_stat", "log residential units"),
         AQUA, -0.27,
         "statutory, same unit offset", contrast=True)
    style_ax(ax)
    ...
    ax.set_xlabel("% change in proactive events per +10pp of tract share",
                  fontsize=10.5)
    ax.set_title("Where the programs go, holding exposure fixed",
    ...
    fig.text(0.01, 0.012,
             "Left: tract-level PPML of agency-initiated complaint counts "
             "2020-2026 on tract demographics (jointly entered), offset by "
             "the margin's exposure (active job-months\nfrom permit spans; "
             "PLUTO residential units), controlling era shares, mean log "
             "building area, commercial share, and log mean units. Open "
             "markers re-run the\ncalendar-driven statutory cycles under "
             "the identical offset. Right: LPM among each lot's first "
             "caller-reported category-30 complaint, controlling priority,\n"
             "building traits, and an active-permit flag, borough x month "
             "FE, SEs clustered by tract. Whiskers are 95% CIs. Agency "
             "events geo-match to tracts at 94.1%.",
...
def print_verdict(est: pd.DataFrame) -> None:
    ...
    blocks = [
        ("discretionary construction / job-month offset",
         pick(est, "allocation", "n_disc_constr")),
        ("7G sweeps / residential-unit offset",
         pick(est, "allocation", "n_7g")),
        ("statutory contrast / residential-unit offset",
         pick(est, "allocation", "n_stat", "log residential units")),
        ("statutory contrast / job-month offset",
         pick(est, "allocation", "n_stat",
              "log active job-months 2020-26")),
    ]
    ...
    # spec-matched, direction-aware placebo reading: a discretionary
    # loading is discounted only when the statutory contrast under the
    # IDENTICAL offset loads with the same sign and its CI excludes 0
    # (opposite-signed statutory loadings cut the other way)
    stat_for = {
        "n_disc_constr": pick(est, "allocation", "n_stat",
                              "log active job-months 2020-26"),
        "n_7g": pick(est, "allocation", "n_stat", "log residential units"),
    }
    print("\n== Plain verdict (spec-matched statutory placebo) ==")
    for outcome, name in [("n_disc_constr", "discretionary construction"),
                          ("n_7g", "7G sweeps")]:
        g, s = pick(est, "allocation", outcome), stat_for[outcome]
        clean, shared = [], []
        for t in DEMOS:
...
def main() -> None:
    ...
    meta = {
        ...
        "title": ("Per active job, construction oversight tilts toward "
                  "Black, Hispanic, and Asian tracts"),
        "subtitle": ("agency-initiated DOB activity per unit of exposure "
                     "(left) and follow-through after unstable-building "
                     "reports (right), 2020-2026 · statutory\ncycles under "
                     "identical offsets supply the calendar-driven contrast "
                     "· crisis follow-through is close to flat across "
                     "demographics"),
    }
