...
def run_series(panel, ycol, cluster, series, meta):
    ...
    out["units"] = "caller complaints per lot-month"
...
def make_figure(perm_meta, lpm, est, es_meta):
    ...
    fig, (ax1, ax2) = plt.subplots(
        1, 2, figsize=(12.8, 6.0), dpi=200,
        gridspec_kw={"left": 0.06, "right": 0.985, "top": 0.70,
    ...
    ax2.set_title("Caller complaints around a building's\nfirst sweep",
                  loc="left", fontsize=12, color=INK, weight="semibold", pad=10)
    ...
    fig.text(0.02, 0.905,
             ...
             f"Right: {es_meta['n_treated']:,} lots first swept 2021-01..2025-05 "
             f"vs {es_meta['n_control']:,} not-yet-swept lots matched within "
             f"tract, on blocks with no sweep in the window · lot and "
             f"calendar-month FE · whiskers = 95% CI, SEs clustered by lot\n"
             f"months 0–12 average {a12['value']:+.3f} caller complaints per "
             f"lot-month [{a12['ci_low']:+.3f}, {a12['ci_high']:+.3f}] = "
             f"{pct12:+.0f}% of the {base:.3f} pre-sweep baseline · joint "
             f"pre-trend p {fmt_p(a12['pretrend_p'])}",
...
def main():
    ...
    pan_treated = expand(ev.rename(columns={"bbl_i": "bbl"})[["bbl", "m0"]],
                         1, count_caller)
    pan_controls = expand(controls, 0, count_caller)
    panel = pd.concat([pan_treated, pan_controls], ignore_index=True)
    n_ctrl = controls["bbl"].nunique()
    print(f"panel: {len(panel):,} rows ({len(ev):,} treated + {n_ctrl:,} "
          f"control lots x {2 * WINDOW + 1} months)")
    es_rows = run_series(panel, "y_caller", "bbl", "sweep_focal",
                         {"n_treated": len(ev), "n_control": n_ctrl})

    RM.mkdir(parents=True, exist_ok=True)
    ART.mkdir(parents=True, exist_ok=True)
    cols = ["analysis", "series", "term", "event_time", "value", "se",
            "ci_low", "ci_high", "pval", "null_mean", "null_sd", "n_draws",
            "n_obs", "n_treated", "n_control", "n_blocks", "baseline",
            "cluster", "pretrend_chi2", "pretrend_df", "pretrend_p",
            "decay_first_ci0_month", "units"]
    out = pd.concat([perm_rows, lpm_rows, es_rows], ignore_index=True)
    out = out.reindex(columns=cols)
    out_csv = RM / "proactive_sweep_structure.csv"
    out.to_csv(out_csv, index=False)
    print(f"\nsaved {out_csv} ({len(out)} rows)")

    make_figure(perm_meta, lpm_rows, es_rows,
                {"n_treated": len(ev), "n_control": n_ctrl})

    hl = perm_meta["share_other_lot_pm3d"]
    a12 = es_rows[es_rows["term"] == "avg_months_0_12"].iloc[0]
    ...
    print(f"  #8 substitution: caller complaints {d12} after a first sweep; "
          f"months 0-12 avg {a12['value']:+.4f} "
          f"[{a12['ci_low']:+.4f}, {a12['ci_high']:+.4f}] per lot-month "
          f"({a12['value'] / a12['baseline'] * 100:+.0f}% of the "
          f"{a12['baseline']:.3f} baseline)")
